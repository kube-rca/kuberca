...
Create CrashLoopBackOff deployment and call analyze endpoint.
...
Environment Variables (required unless CLEANUP_ONLY=true):
  ...
  CRASH_COMMAND     Command that will cause Crash (e.g. exit 1)
...
Examples:
  # Basic usage
  DEPLOYMENT_NAME=crash-test IMAGE=python:3.11-alpine ... ./curl-test-crashloop.sh
EOF
}
...
case "$CRASH_COMMAND" in
  *$'\n'*) # This is a literal newline character
    log_error "CRASH_COMMAND must be a single line"
    exit 1
    ;;
esac
...
crash_command_escaped=$(printf '%s' "$CRASH_COMMAND" | sed "s/'/''/g")

log_info "Applying deployment: $DEPLOYMENT_NAME"
log_info "  Namespace: $NAMESPACE"
log_info "  Image: $IMAGE"
log_info "  Command: $CRASH_COMMAND"
...
log_info "Waiting for CrashLoopBackOff/Error... (timeout: ${WAIT_SECONDS}s)"
crash_deadline=$((SECONDS + WAIT_SECONDS))
crash_reason=""
while [ "$SECONDS" -lt "$crash_deadline" ]; do
  reason=$( \
  ...
  if [ "$reason" = "CrashLoopBackOff" ] || [ "$reason" = "Error" ] || [ "$reason" = "ContainerCannotRun" ]; then
    crash_reason="$reason"
    break
  fi
  printf "."
...
if [ -z "$crash_reason" ]; then
  log_error "timed out waiting for CrashLoopBackOff: pod=$pod_name"
  kubectl_local -n "$NAMESPACE" describe pod "$pod_name" || true
  exit 1
fi
log_ok "Crash/Error detected! Reason: $crash_reason"
...
exit_code=$(kubectl_local -n "$NAMESPACE" get pod "$pod_name" -o jsonpath='{.status.containerStatuses[0].lastState.terminated.exitCode}' 2>/dev/null || true)
...
description="Deployment ${DEPLOYMENT_NAME} pod ${pod_name} is in CrashLoopBackOff"
now="$RUN_TIMESTAMP"

log_info "  Pod: $pod_name"
log_info "  Reason: $crash_reason"
...
# Here we could construct a payload similar to OOMKilled but adapted for CrashLoop
# For now, we will skip the detailed payload construction unless requested,
# as the user primarily asked for the scenario setup.
# But to be complete, I will add a basic payload.
...
payload=$(cat <<JSON
{
  "alert": {
    ...
    "annotations": {
      ...
      "exit_code": "$(json_escape "$exit_code")",
      "reason": "$(json_escape "$crash_reason")"
    },
    "startsAt": "$now",
    "endsAt": "$now",
    "generatorURL": ""

...
- Backend: Alertmanager Webhook 수신 및 Slack 스레드 알림
  ...
  - 수동 분석 트리거 API (`/api/v1/incidents/:id/analyze`, `/api/v1/alerts/:id/analyze`)
  - Hidden Incident 관리 API(`/api/v1/incidents/hidden`, `/api/v1/incidents/:id/unhide`)
  - Alert 수동 해제 API(`/api/v1/alerts/:id/resolve`, `/api/v1/alerts/bulk-resolve`)
  - Feedback API(incident/alert 투표/코멘트)
  - Webhook Settings CRUD API(`/api/v1/settings/webhooks*`)
  - App Settings API(`/api/v1/settings/app*`) - Notification, Flapping, AI Provider, Analysis Mode
...
- Agent: FastAPI 기반 분석/요약/채팅 API
  ...
  - node-level alert 지원: node status/metrics, cluster events 기반 분석 보강
  - Provider API key 미설정 시 fallback 응답 반환
- Frontend: Incident/Alert 운영 UI
  - 인증/로그인 + OIDC
  - Incident/Alert/Hidden 상세 관리 + Alert 분석 이력(firing/resolved) 분리 표시
  - Alert 수동 해제 (단건 Resolve + Bulk Resolve 최대 50건)
  - 유사 Incident 검색(`/api/v1/embeddings/search`)
  - Feedback UI, Webhook Settings UI, Floating Chat 패널
  - Analysis Dashboard(`/analysis`) 기반 추이/분포 시각화
  - Settings UI: Notification, Flapping Detection, AI Provider, Analysis Mode
  - SSE 실시간 갱신 + polling fallback
- Helm: backend/agent/frontend 배포용 `kube-rca` 차트
- DB: PostgreSQL + pgvector (`incidents`, `alerts`, `alert_analyses`, `alert_analysis_artifacts`, `embeddings`, `feedback_*`, `webhook_configs`, `app_settings`)

상세 런타임 흐름은 [ARCHITECTURE.md](ARCHITECTURE.md), 다이어그램은 [diagrams/](diagrams/)를 참고합니다.
...
5. 운영자가 수동 분석 트리거와 Analysis Mode 설정으로 auto/manual 분석 정책을 제어
6. Alert 수동 해제(단건/Bulk) 시 Slack "[Manually Resolved]" 알림 + Agent 분석(단건만)
7. 운영자가 Feedback(투표/코멘트), Chat, Webhook Settings, App Settings로 운영 품질 개선
8. Analysis Dashboard와 SSE + polling fallback으로 UI 상태와 추이를 실시간/주기적으로 확인
...
- node-level alert의 경우 node 상태와 cluster events까지 분석 범위를 확장
...
### 4.5 운영 정책 및 관측 제어
- Analysis Mode와 App Settings로 auto/manual 분석 정책과 운영 파이프라인을 조정
- Analytics Dashboard로 incident/alert 추이, severity 분포, namespace 집중도를 확인
...
- Analytics Dashboard와 Settings UI로 운영 판단 및 정책 조정 비용 감소
...
- Tempo 연동(선택)
- Kubernetes node/pod/workload 상태 및 cluster events 활용
...
- Chat/Settings/Analytics UI를 통한 운영 액션 집중화

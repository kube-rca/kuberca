...
3. Agent API는 별도 서비스로 존재하지만 Backend와의 연동은 아직 구현되지 않았습니다.
...
- Agent (Gin, 기본 포트 `:8082`)
  - `GET /ping`, `GET /healthz`, `GET /`, `POST /analyze/alertmanager`
  - 분석 응답은 placeholder 문자열을 반환합니다.
- Frontend (React)
  - `frontend/src/utils/api.ts` 기준으로 `GET /api/rca`를 호출합니다.

## 미구현/계획

- Backend ↔ Agent 연동, RCA 결과 저장/조회 API
- Incident Store, Vector DB, LLM 연동
